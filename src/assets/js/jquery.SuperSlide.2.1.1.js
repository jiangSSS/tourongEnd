...
! function(a) {
	a.fn.slide = function(b) {
		return a.fn.slide.defaults = {
			...
			if (0 == k && (k = m), R && (k = 2), x) {
				if (m >= u)
					if ("leftLoop" == e || "topLoop" == e) k = 0 != m % t ? (0 ^ m / t) : m / t;
					else {
						var V = m - u;
						k = 1 + parseInt(0 != V % t ? V / t*0.5 : V / t*0.5), 0 >= k && (k = 1)
					}
				else k = 1;
				j.html("");
				var W = "";
				if (1 == c.autoPage || "true" == c.autoPage)
					for (var X = 0; k > X; X++) W += "<li>" + (X) + "</li>";
				else
					for (var X = 0; k > X; X++) W += c.autoPage.replace("$", X);
				j.html(W);
				var j = j.children()
			}
			if (m >= u) {
				...
				switch (e) {
					...
					case "top":
						...
						break;
					case "left":
						l.wrap('<div class="tempWrap" style="overflow:hidden; position:relative; width:1200px"></div>').css({
							width: m * D,
							left: -(p * t) * D,
							position: "relative",
							overflow: "hidden",
					...
					case "leftLoop":
					case "leftMarquee":
						Z(), l.wrap('<div class="tempWrap" style="overflow:hidden; position:relative; width:1200px"></div>').css({
							width: (m + u + P) * D,
							position: "relative",
							overflow: "hidden",
							padding: "0",
