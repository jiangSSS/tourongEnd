        ...
        methods: {
            submit() {
                ...
                if (this.formData.content != '') {
                    this.$axios.get('/jsp/wap/center/do/doEditProjectDynamic.jsp', { params: { projectId: id, content: this.formData.content, imgPaths } }).then(res => {
                        if (res.success == 'true') {
                            let instance = Toast('提交进展成功');
                            setTimeout(() => {
                                instance.close();
                            }, 1000);
                            setTimeout(() => {
